...
def decode (tuplesfile, *mapnames):
    maps = [[y.strip() for y in open(x).readlines()] for x in mapnames]
    result = []
    for line in open(tuplesfile):
        if '#' in line:
...
def main(argv):
    ...
    if len(argv) < 3:
        print("Usage: python3 ./print_bddbddb_results.py <main classname> <tuples_filename>")
        return 0
